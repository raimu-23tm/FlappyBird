...
class GameScene: SKScene, SKPhysicsContactDelegate  {
    ...
    func setupItem() {
        ...
        let createItemAnimation = SKAction.run({
            ...
            // 0〜random_y_rangeまでのランダム値を生成
            let random_y = CGFloat.random(in: 350 ..< 350 + random_y_range)
            
            // アイテム関連のノードを乗せるノードを作成
            let item = SKNode()
            item.position = CGPoint(x: self.frame.size.width ,
                                    y: random_y)
            item.zPosition = -50
            ...
            // スプライトに物理演算を設定する
            item.physicsBody = SKPhysicsBody(rectangleOf:
                CGSize(width:itemTexture.size().width / 2,
                       height:itemTexture.size().height / 2))
        ...
        Timer.scheduledTimer(withTimeInterval: 1.2,repeats: false,
                             block: { _ in self.itemCreateRun(_ : repeatForeverAnimation)
         })

    }
    
    //アイテム生成自動実行を起動
    func itemCreateRun(_ repeatForeverAnimation : SKAction) {
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory ||
            ...
            if (secondNode?.isHidden == false )
            {
                ...
                do {
                    // 音声ファイル取得
                    let path = Bundle.main.path(forResource: "decision29", ofType: "mp3")!
